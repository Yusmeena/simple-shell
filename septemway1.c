...
/**
 * list_to_array - returns an array of strings from the list->txt
...
{
	list_t *list = head;
	size_t list_size = list_length(head), index;
	char **strings_array;
	char *text;
	...
	{
		text = malloc(string_length(list->txt) + 1);
		if (!text)
		{
		...
		text = copy_string(text, list->txt); /* Copy content of 'list->txt' to 'text' */
...
size_t display_list(const list_t *h)
{
	...
	while (h)
	{
		putin(change_num(h->num, 10, 0)); /* Print the modified value of 'h->num' */
...
/**
 * find_node_starts_with - returns a node whose string starts with prefix
 ...
 */
list_t *find_node_starts_with(list_t *list, char *prefix, char c)
{
	char *q = NULL;
